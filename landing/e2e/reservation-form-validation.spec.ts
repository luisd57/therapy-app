...
test.describe('Reservation form - validation', (): void => {
  test('empty required fields block submission', async ({ page }: PlaywrightTestArgs): Promise<void> => {
  ...
  test('malformed email is rejected by the browser', async ({ page }: PlaywrightTestArgs): Promise<void> => {
    await gotoSlotBrowser(page);
    await selectFirstAvailableSlot(page);
