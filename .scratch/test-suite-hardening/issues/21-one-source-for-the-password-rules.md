...
**What to build:** the password rules exist once in the API. Whoever changes a
...
constraint, used by registration and by password reset. Both check a length of 8
to 72 and four character classes, with the five message strings retyped
identically. Nothing fails if one copy drifts from the other.
...
Ticket 04 already covers both copies rule by rule, so the refactor is checked by
tests that can see every rule.

**Not in scope:** the dashboard holds a third copy of the same rules, with the
length bound split into two checks. Sharing rules between the API and an Angular
...
- [ ] One implementation of the rules and their message strings remains, and `PasswordValidator` is deleted
